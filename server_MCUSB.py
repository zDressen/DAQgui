...
def MCUSB_reinit(high_channel,low_channel, sample_rate, input_mode):
    ...
    if high_channel >= number_of_channels:
        high_channel = number_of_channels - 1
    channel_count = high_channel - low_channel + 1
    print(str(high_channel) + " " + str(low_channel) + " " + str(channel_count))
    samples_per_channel = DAQ.samples_per_channel
...
while True:
    ...
    if len(waveform_data) > 1:
        ...
        elif waveform_data[0] == 'Reinit':
            socket.send(b"Reply: Reinitiating Box")
            waveform_data.pop(0)
            MCUSB_reinit(int(waveform_data[0]),int(waveform_data[1]),int(waveform_data[2]),waveform_data[3])
